...
def navigate_wait_range(x=0, y=0, z=0, speed=0.5, frame_id='', auto_arm=False, tolerance=0.2):
    navigate(x=x, y=y, z=z, speed=speed, frame_id=frame_id, auto_arm=auto_arm)
    while not rospy.is_shutdown():
        telem = get_telemetry(frame_id='navigate_target')
        if math.sqrt(telem.x ** 2 + telem.y ** 2 + telem.z ** 2) < tolerance:
        ...
        if dist_z < 0.94:
            ...
            navigate_wait_range(x=x, y=y, z=z, speed = speed, frame_id=frame_id)
            break
        rospy.sleep(0.2)
...
def main():
    ...
    navigate_wait_range(x=7,y=0.97,z=1.5,speed = 0.1,frame_id='aruco_map')
    navigate_wait(x=8.9,y=1.4,z=1.5,speed = 0.1,frame_id='aruco_map')
